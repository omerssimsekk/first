...
const CustomMarker = ({ category, onPress }) => {
  const markerInfo = CATEGORIES[category] || { color: theme.colors.primary, icon: 'location' };
...
const LocationsScreen = () => {
  ...
  // Load saved markers when component mounts
  useEffect(() => {
    loadSavedMarkers();
  }, []);
  ...
  const loadSavedMarkers = async () => {
    try {
      ...
      if (storedMarkers) {
        setSavedMarkers(JSON.parse(storedMarkers));
      }
    } catch (error) {
  ...
  const handleDeleteMarker = (markerId) => {
    setSavedMarkers((prevMarkers) => prevMarkers.filter(marker => marker.id !== markerId));
